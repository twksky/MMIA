...
@interface MMiaClothesPageController () <CHTCollectionViewDelegateWaterfallLayout,UICollectionViewDataSource, MMiaErrorTipViewDelegate>
{
    BOOL _isLoadding;
    BOOL _isRefresh;
...
- (void)getClothesDataForRequest:(NSInteger)start size:(NSInteger)size;
...
@implementation MMiaClothesPageController
...
- (void)viewDidLoad
{
    ...
    [self getClothesDataForRequest:0 size:Request_Data_Count];
    [self initTitle];
}
- (void)initTitle{
    ...
    [app.mmiaDataEngine startAsyncRequestWithUrl:MMia_HOMEPAGE_CHANNEL_URL param:dict requestMethod:HTTP_REQUEST_POST completionHandler:^(NSDictionary *jsonObject)
     {
         ...
         {
            // NSLog( @"clothesArray = %@", jsonObject[@"data"] );
             NSMutableArray* clothArr = [NSMutableArray arrayWithArray:self.dataArr];
             if( _isRefresh )
             {
                 [clothArr removeAllObjects];
             }
             ...
             for( NSDictionary *dict in jsonObject[@"data"])
             {
                 ...
                 [clothArr addObject:item];
             }
             [self refreshPageData:clothArr downNum:[jsonObject[@"num"] intValue]];
         }
         else
         {
...
- (void)onErrorTipViewRefreshBtnClicked:(MMiaErrorTipView* )sender
{
    ...
    [self getClothesDataForRequest:self.dataArr.count size:Request_Data_Count];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (headViewHeight1 == 0) {
        headViewHeight1 = point.y;
    }
    float titleAlpha = 1 - point.y / headViewHeight1;
    if (titleAlpha <= 1){
...
- (void)addRefreshHeader
{
    __block MMiaClothesPageController* clothesVC = self;
    [_collectionView addHeaderWithCallback:^{
        if( clothesVC->_isLoadding )
            return;
        
        if( clothesVC->_showErrTipView )
        {
            [MMiaErrorTipView hideErrorTipForView:clothesVC.collectionView];
        }
        clothesVC->_isRefresh = YES;
        clothesVC->_isLoadding = YES;
        [clothesVC getClothesDataForRequest:0 size:Request_Data_Count];
    }];
}
...
- (void)addRefreshFooter
{
    ...
    [_collectionView addFooterWithCallback:^{
        ...
        clothesVC->_isRefresh = NO;
        clothesVC->_isLoadding = YES;
        [clothesVC getClothesDataForRequest:clothesVC.dataArr.count size:Request_Data_Count];
    }];
}
