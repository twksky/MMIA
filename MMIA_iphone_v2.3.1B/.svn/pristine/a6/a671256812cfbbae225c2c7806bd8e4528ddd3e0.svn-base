...
@interface MMiaFoodPageController () <CHTCollectionViewDelegateWaterfallLayout,UICollectionViewDataSource, MMiaErrorTipViewDelegate>
{
    BOOL _isLoadding;
    BOOL _isRefresh;
...
- (void)getFoodDataForRequest:(NSInteger)start size:(NSInteger)size;
...
@implementation MMiaFoodPageController
...
- (void)viewDidLoad
{
    ...
    [self getFoodDataForRequest:0 size:Request_Data_Count];
    [self initTitle];
}
    ...
    [app.mmiaDataEngine startAsyncRequestWithUrl:MMia_HOMEPAGE_CHANNEL_URL param:dict requestMethod:HTTP_REQUEST_POST completionHandler:^(NSDictionary *jsonObject)
     {
         ...
         {
            // NSLog( @"foodArray = %@", jsonObject[@"data"] );
             NSMutableArray* foodArr = [NSMutableArray arrayWithArray:self.dataArr];
             if( _isRefresh )
             {
             ...
             for( NSDictionary *dict in jsonObject[@"data"])
             {
                 ...
                 [foodArr addObject:item];
             }
             [self refreshPageData:foodArr downNum:[jsonObject[@"num"] intValue]];
         }
         else
         {
...
- (void)onErrorTipViewRefreshBtnClicked:(MMiaErrorTipView* )sender
{
    ...
    [self getFoodDataForRequest:self.dataArr.count size:Request_Data_Count];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (headViewHeight2 == 0) {
        headViewHeight2 = point.y;
    }
    float titleAlpha = 1 - point.y / headViewHeight2;
    if (titleAlpha <= 1){
...
- (void)addRefreshHeader
{
    __block MMiaFoodPageController* foodVC = self;
    [_collectionView addHeaderWithCallback:^{
        if( foodVC->_isLoadding )
            return;
        
        if( foodVC->_showErrTipView )
        {
            [MMiaErrorTipView hideErrorTipForView:foodVC.collectionView];
        }
        foodVC->_isRefresh = YES;
        foodVC->_isLoadding = YES;
        [foodVC getFoodDataForRequest:0 size:Request_Data_Count];
    }];
}
...
- (void)addRefreshFooter
{
    ...
    [_collectionView addFooterWithCallback:^{
        if( foodVC->_isLoadding )
            return;
        foodVC->_isRefresh = NO;
        foodVC->_isLoadding = YES;
        [foodVC getFoodDataForRequest:foodVC.dataArr.count size:Request_Data_Count];
    }];
}
