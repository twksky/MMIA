...
- (UILabel *)lastUpdateTimeLabel
{
    if (!_lastUpdateTimeLabel) {
        ...
        lastUpdateTimeLabel.textColor = MMIATextAlignmentLeft;
...
- (void)layoutSubviews
{
    ...
    CGFloat statusY = CGRectGetMaxY(self.arrowImage.frame);
    CGFloat statusX = CGRectGetMaxX(self.activityView.frame) + 5;
   
    CGFloat statusHeight = self.height - statusY;
    CGFloat statusWidth = self.width - statusX;
    // 1.状态标签
...
- (void)setState:(MJRefreshState)state
{
    ...
    if (MJRefreshStateNormal == state && MJRefreshStateRefreshing == oldState)
    {
