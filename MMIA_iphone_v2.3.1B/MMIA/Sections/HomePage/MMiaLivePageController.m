...
@interface MMiaLivePageController () <CHTCollectionViewDelegateWaterfallLayout,UICollectionViewDataSource, MMiaErrorTipViewDelegate>
{
    BOOL _isLoadding;
    BOOL _isRefresh;
...
- (void)getLiveDataForRequest:(NSInteger)start size:(NSInteger)size;
...
@implementation MMiaLivePageController
...
- (void)viewDidLoad
{
    ...
    [self getLiveDataForRequest:0 size:Request_Data_Count];
    [self initTitle];
}
    ...
    [app.mmiaDataEngine startAsyncRequestWithUrl:MMia_HOMEPAGE_CHANNEL_URL param:dict requestMethod:HTTP_REQUEST_POST completionHandler:^(NSDictionary *jsonObject)
     {
         ...
         {
             //NSLog( @"liveArray = %@", jsonObject[@"data"] );
             NSMutableArray* liveArr = [NSMutableArray arrayWithArray:self.dataArr];
             if( _isRefresh )
             {
                 [liveArr removeAllObjects];
             }
             ...
             for( NSDictionary *dict in jsonObject[@"data"])
             {
                 ...
                 [liveArr addObject:item];
             }
             [self refreshPageData:liveArr downNum:[jsonObject[@"num"] intValue]];
         }
         else
         {
...
- (void)onErrorTipViewRefreshBtnClicked:(MMiaErrorTipView* )sender
{
    ...
    [self getLiveDataForRequest:self.dataArr.count size:Request_Data_Count];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (headViewHeight3 == 0) {
        headViewHeight3 = point.y;
    }
    float titleAlpha = 1 - point.y / headViewHeight3;
    if (titleAlpha <= 1){
...
- (void)addRefreshHeader
{
    __block MMiaLivePageController* liveVC = self;
    [_collectionView addHeaderWithCallback:^{
        if( liveVC->_isLoadding )
            return;
        
        if( liveVC->_showErrTipView )
        {
            [MMiaErrorTipView hideErrorTipForView:liveVC.collectionView];
        }
        liveVC->_isRefresh = YES;
        liveVC->_isLoadding = YES;
        [liveVC getLiveDataForRequest:0 size:Request_Data_Count];
    }];
}
...
- (void)addRefreshFooter
{
    ...
    [_collectionView addFooterWithCallback:^{
        if( liveVC->_isLoadding )
            return;
        liveVC->_isRefresh = NO;
        liveVC->_isLoadding = YES;
        [liveVC getLiveDataForRequest:liveVC.dataArr.count size:Request_Data_Count];
    }];
}
