...
//            NSLog( @"res = %@", responseDic );
            ...
            // paperBigPicture
            self.paperViewController.topDataArray = self.homeModel.recommendList;
            self.paperViewController.itemsArray = self.homeModel.goodRecommendList;
            
            //lx add 判断是否需要加载更多
            self.paperViewController.everyNum = self.homeModel.goodRecommendList.count;
            // paperDetailVC
            self.paperViewController.detailViewController.everyNum = self.homeModel.goodRecommendList.count;
            self.paperViewController.detailViewController.dataArray = self.homeModel.goodRecommendList;
            
            if (self.paperViewController.detailViewController.collectionView) {
                [self.paperViewController.detailViewController.collectionView reloadData];
            }
            
            self.tableView.scrollEnabled = YES;
            [self.tableView reloadData];
            [self loadDetailViewControllerImageData];
        }
        else
        {
            // 错误处理
...
#pragma mark ** 缓存详情页每个item第一张图片

- (void)loadDetailViewControllerImageData
{
    for( MmiaPaperProductListModel* model in self.homeModel.goodRecommendList )
    {
        if( model.productPictureList.count > 0 )
        {
            MmiaProductPictureListModel* pictureModel = model.productPictureList[0];
        //[SDWebImageManager.sharedManager downloadImageWithURL:NSURLWithString(pictureModel.picUrl) options:0 progress:nil completed:nil];
        }
    }
}
...
- (MmiaPaperViewController *)paperViewController
{
    if( !_paperViewController )
    {
        ...
        //lx add 添加推荐位跳转
        WeakSelf(weakSelf);
        _paperViewController.TopViewTapActionBlock = ^(NSInteger index){
            
            if (index < weakSelf.homeModel.recommendList.count)
            {
                MmiaPaperRecommendModel *model = weakSelf.homeModel.recommendList[index];
                [weakSelf pushViewControllerWithSourceId:model.sourceId type:model.type];
            }
        };
        // lx end
...
- (void)addNavigationBarItem
{
    ...
    [logoImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.height.equalTo(self.naviBarView);
        make.width.equalTo(@(image.size.width));
    }];
    
    [cateButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.top.equalTo(self.naviBarView);
        make.width.equalTo(@(butImage.size.width + 20));
        make.height.equalTo(@(kNavigationBarHeight));
    }];
    
    [searchButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(cateButton.mas_left);
        make.top.equalTo(self.naviBarView);
        make.width.equalTo(@(srcImage.size.width + 20));
        make.height.equalTo(@(kNavigationBarHeight));
    }];
...
- (void)searchButtonClick:(UIButton *)sender
{
    ...
    MmiaSearchViewController* searchVC = [[MmiaSearchViewController alloc] initWithCollectionViewLayout:layout];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch ( row )
    {
        case 0:
        {
            ...
            [moreCell.leftImageView sd_setImageWithURL:NSURLWithString(leftItem.pictureUrl) placeholderImage:nil];
            [moreCell.rightTopImageView sd_setImageWithURL:NSURLWithString(rightTopItem.pictureUrl) placeholderImage:nil];
            [moreCell.rightBottomImageView sd_setImageWithURL:NSURLWithString(rightBottomItem.pictureUrl) placeholderImage:nil];
        ...
        case 1:
        {
            ...
            [onlyCell.onlyImageView sd_setImageWithURL:NSURLWithString(onlyItem.pictureUrl) placeholderImage:nil];
        ...
        case 2:
        {
            ...
            [twoCell.leftImageView sd_setImageWithURL:NSURLWithString(leftItem.pictureUrl) placeholderImage:nil];
            [twoCell.rightImageView sd_setImageWithURL:NSURLWithString(rightItem.pictureUrl) placeholderImage:nil];
