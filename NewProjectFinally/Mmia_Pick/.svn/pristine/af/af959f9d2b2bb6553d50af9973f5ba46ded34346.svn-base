...
- (void)getHomeRecommendListWithStart: (NSInteger)start
{
    ...
    [[MMiaNetworkEngine sharedInstance]startPostAsyncRequestWithUrl: MmiaHomeRecommendListURL param:infoDict completionHandler:^(AFHTTPRequestOperation *operation, NSDictionary* responseDic){
        ...
        if( ![responseDic[@"status"] intValue] )
        {
            ...
            self.detailViewController.everyNum = self.everyNum;
             self.detailViewController.dataArray = self.itemsArray;
            if (_addMoreData)
            {
...
- (MmiaPaperDetailsViewController *)detailViewController
{
    if( !_detailViewController )
    {
        MmiaCollectionViewLargeLayout* largeLayout = [[MmiaCollectionViewLargeLayout alloc] init];
        _detailViewController = [[MmiaPaperDetailsViewController alloc] initWithCollectionViewLayout:largeLayout];
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //加载更多数据
    self.detailViewController.AddMoreDataBlock = ^(MmiaPaperDetailsViewController *detailVC)
    {
    ...
    {
         self.detailViewController.selectIndexPath = indexPath;
         self.detailViewController.dataArray = self.itemsArray;
