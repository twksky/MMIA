...
- (void)_initialize
{
    ...
    _alignment = MPageControlAlignmentCenter;
    _verticalAlignment = MPageControlVerticalAlignmentMiddle;
...
- (CGFloat)_leftOffset
{
    ...
    switch (_alignment)
    {
        case MPageControlAlignmentCenter:
            left = CGRectGetMidX(rect) - (size.width / 2.0f);
            break;
        case MPageControlAlignmentRight:
...
- (CGFloat)_topOffset
{
    ...
    {
        case MPageControlVerticalAlignmentMiddle:
            top = CGRectGetMidY(rect) - (_measuredIndicatorHeight / 2.0f);
            break;
        case MPageControlVerticalAlignmentBottom:
...
- (void)setCurrentPage:(NSInteger)currentPage sendEvent:(BOOL)sendEvent canDefer:(BOOL)defer
{
    ...
    if (!self.defersCurrentPageDisplay || !defer)
    {
        _displayedPage = _currentPage;
        [self setNeedsDisplay];
    }
