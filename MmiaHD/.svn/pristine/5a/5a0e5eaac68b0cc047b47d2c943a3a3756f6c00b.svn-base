...
- (void)layoutSubviews
{
    ...
    frame.origin.y = (self.height - self.activityView.image.size.height) / 2;
    frame.origin.x += 10;
    self.activityView.frame = frame;
    
    CGFloat statusY = CGRectGetMinY(self.bounds);
    CGFloat statusX = CGRectGetMaxX(self.activityView.frame) + 5;
    CGFloat statusWidth = self.width - statusX;
    
    self.statusLabel.frame = CGRectMake(statusX, statusY, statusWidth, self.height - statusY);
