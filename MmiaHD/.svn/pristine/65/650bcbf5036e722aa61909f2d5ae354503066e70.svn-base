...
-(UILabel *)nikeNameLabel
{
    if (!_nikeNameLabel)
    {
        ...
        _nikeNameLabel.frame = CGRectMake(0,CGRectGetMaxY(self.headImageView.frame) + 7, _setBackView.width, 25);
...
- (void)resetFrame:(CGRect)frame
{
    ...
    _setBackView.frame = CGRectMake((self.width- 400)/2, 0, 400, 180);
...
-(void)resetSubViewsWithDictionary:(LoginInfoItem *)loginItem
{
    ...
    if (loginItem.userType == 0)
    {
        ...
        self.nikeNameLabel.frame = CGRectMake((CGRectGetWidth(_setBackView.frame) - width)/2.0,CGRectGetMaxY(self.headImageView.frame) + 7, width, 25);
