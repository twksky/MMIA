...
-(void)shareClick:(UIButton *)btn{
    ...
    [_menuBar initWithTarget:_target];
...
{
    if (btn.tag == 11) {
        [_target performSelector:@selector(WXShare)];
    }
    if (btn.tag == 12) {
        [_target performSelector:@selector(FriendShare)];
    }
    if (btn.tag == 13) {
        [_target performSelector:@selector(WBShare)];
    }
    if (btn.tag == 14) {
        [_target performSelector:@selector(QQShare)];
    }
