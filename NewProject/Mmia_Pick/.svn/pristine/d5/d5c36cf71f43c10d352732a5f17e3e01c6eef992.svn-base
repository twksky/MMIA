...
//    self.brandDescription.Frame = CGRectMake(Gap, Gap, self.frame.size.width-20, 1);
    ...
    self.brandDescription.text = data.brandDescriptionText;
    ...
    CGSize officalWebsiteSize = CGSizeMake(ScreenWidth-HeadGap-HeadGap, MAXFLOAT);
    CGRect officalWebsiteRect = [self.officalWebsite.text boundingRectWithSize:officalWebsiteSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.officalWebsite.font} context:nil];
    [self.officalWebsite setFrame:CGRectMake(HeadGap, HeadGap+H1+H2, ScreenWidth-HeadGap-HeadGap, officalWebsiteRect.size.height)];
    [self addSubview:self.officalWebsite];

    
    //官网的名字
    [self.officalWebsiteName setTitle:data.officalWebsiteNameText forState:UIControlStateNormal];
    [self.officalWebsiteName setTitleColor:ColorWithHexRGB(0x666666) forState:UIControlStateNormal];
    //    [_officalWebsiteName setTitleColor:ColorWithHexRGB(0x333333) forState:UIControlStateSelected];
    ...
    CGSize titleSize = CGSizeMake(ScreenWidth-HeadGap-HeadGap, MAXFLOAT);
    CGRect titleRect = [self.title.text boundingRectWithSize:titleSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.title.font} context:nil];
    [self.title setFrame:CGRectMake(HeadGap, HeadGap+(H1)+(H2)+(H3)+8+(H4)+HeadGap+HeadGap, ScreenWidth-HeadGap-HeadGap, titleRect.size.height)];
